...
const screenUpdate = () => {
  if (folders.length == 0) {
    //FOR THE CASE WHERE THE OBJECT IS EMPTY (all objects removed)
    projects.innerHTML = "";
    ...
    folders.forEach((item, index) => {
      if (folders.indexOf(activeFolder) == index) {
        //to put 'class="activeFolder' to the right div
...
const screenUpdateSee = (event) => {
  inputs.innerHTML = "";
  inputs.innerHTML +=
    `<div class="testInfo"><h3>Description:</h3><p>${
      activeFolder[event.target.parentNode.dataset.indexNumber].description
    }</p></div>` +
    `<div class="testdueDate"><h3>dueDate:</h3><p>${
      activeFolder[event.target.parentNode.dataset.indexNumber].dueDate
    }</p></div>` +
    `<div class="testpriority"><h3>Priority:</h3><p>${
      activeFolder[event.target.parentNode.dataset.indexNumber].priority
    }</p></div>`;
...
const screenUpdateEdit = (event) => {
  ...
  titleEdit.setAttribute(
    "value",
    `${activeFolder[event.target.parentNode.dataset.indexNumber].title}`
  );
  titleEdit.setAttribute(
    "data-index-number",
    `${event.target.parentNode.dataset.indexNumber}`
  );
  descriptionEdit.innerHTML = `${
    activeFolder[event.target.parentNode.dataset.indexNumber].description
  }`;
  dateEdit.setAttribute(
    "value",
    `${activeFolder[event.target.parentNode.dataset.indexNumber].dueDate}`
  );

  if (
    activeFolder[event.target.parentNode.dataset.indexNumber].priority == "Low"
  ) {
    priorityEdit.innerHTML =
      '<option value="High">High</option><option value="Medium">Medium</option><option value="Low" selected>Low</option>';
  } else if (
    activeFolder[event.target.parentNode.dataset.indexNumber].priority ==
    "Medium"
  ) {
    ...
    activeFolder[event.target.parentNode.dataset.indexNumber].priority == "High"
  ) {
    priorityEdit.innerHTML =
  ...
  if (
    activeFolder[event.target.parentNode.dataset.indexNumber].check == "Done"
  ) {
    statusEdit.innerHTML =
      '<option value="Not Done">Not Done</option><option value="Done" selected>Done</option>';
  } else if (
    activeFolder[event.target.parentNode.dataset.indexNumber].check ==
    "Not Done"
  ) {
    statusEdit.innerHTML =
      '<option value="Not Done" selected>Not Done</option><option value="Done">Done</option>';
  }
};
